...
def initial_position_svg(
    ...
    def molar_arrow(val: float, side: str, y: int) -> str:
        ...
        arrow_length = min(80, abs(val) * 24)  # Longer arrows
        
        if val > 0:  # Molar shifted mesially
            color = "#e74c3c"  # Red for mesial
            if side == "R":
                x1, x2 = x - 12, x - 12 + arrow_length
            else:
                x1, x2 = x + 12, x + 12 - arrow_length
        else:  # Molar shifted distally
...
def movement_sign(rem: float, side: str, tooth_type: str) -> float:
    ...
    # EXTRACTION/SPACING: Different patterns by tooth type
        ...
        if tooth_type == "6":  # MOLARS
            ...
            if side == "R":
                return 1.0   # Right molar moves right (mesially)
            else:
                return -1.0  # Left molar moves left (mesially)
        else:  # CANINES and INCISORS
            # Anterior teeth move DISTALLY (toward molars)
            if side == "R":
                return -1.0  # Right canine/incisor moves left (distally)
            else:
                return 1.0   # Left canine/incisor moves right (distally)
...
st.markdown("<div class='small-muted'>Lower arch calculated with dental + skeletal midline tracking. <b>Growth prediction integrated via CVMS staging.</b></div>", unsafe_allow_html=True)

tabs = st.tabs(["Step 1: Initial Tooth Positions", "Step 1B: Growth Assessment", "Step 2: Lower Arch Discrepancy", "Step 3: Determining Movement"])
...
with tabs[2]:
    ...
    with col1:
        st.markdown("**Growth**")
    with col2:
        st.markdown(f"<div style='text-align: center; color: purple; font-weight: bold;'>{growth_L_33:.1f}</div>", unsafe_allow_html=True)
    with col3:
        st.markdown(f"<div style='text-align: center; color: purple; font-weight: bold;'>{growth_L_33:.1f}</div>", unsafe_allow_html=True)
    with col4:
        st.markdown(f"<div style='text-align: center; color: purple; font-weight: bold;'>{growth_L_77:.1f}</div>", unsafe_allow_html=True)
    with col5:
        st.markdown(f"<div style='text-align: center; color: purple; font-weight: bold;'>{growth_L_77:.1f}</div>", unsafe_allow_html=True)
